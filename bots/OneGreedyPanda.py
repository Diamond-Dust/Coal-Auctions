...
class OneGreedyPanda(BaseBot):
    ...
    def calculate_bets(self, quality_tests):
        ...
        if self.t > 0:
            panda_greed = self.max_sum / self.t
            my_bet = [
                Bet(
                    prognosis[0],
                    max(prognosis[1] - panda_greed, 10),
                    index
    ...
    def historical_income(self, mine_id, prognosis):
        ...
        try:
            ...
            x = [(data[1], data[1]**2, data[1]**3) for data in income_data] # quality tests
    ...
    def update_historical_income(self, results, tests):
        ...
        self.max_sum += max(actual_incomes)
